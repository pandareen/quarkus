...
For HTTP/1.x, Quarkus replaces that handler with a subclass that logs a **WARN** (category `io.quarkus.vertx.http.runtime.netty.LoggingHttpContentDecompressor`) if decompression fails, before the connection is torn down.
...
* `snappy` using Netty's Snappy *framing* format (`SnappyFrameDecoder`). Payloads must be valid framed Snappy (for example as emitted by Netty's `SnappyFrameEncoder`). Raw Snappy block compression without the framing envelope is not accepted; decompression failures can result in the connection being closed without an HTTP response. Supported on the JVM and in GraalVM native images when inbound decompression is enabled.
