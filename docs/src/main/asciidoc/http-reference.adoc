...
* `snappy` using Netty's Snappy *framing* format (`SnappyFrameDecoder`) on the JVM only. Payloads must be valid framed Snappy (for example as emitted by Netty's `SnappyFrameEncoder`). Raw Snappy block compression without the framing envelope is not accepted; decompression failures on HTTP/1.x produce `400 Bad Request` with a short plain-text body (and a WARN log) when the decompressor runs; on HTTP/2 the connection may still be closed without a normal HTTP response. GraalVM native images do not wire Snappy inbound decoding (the body is not decoded).
