...
* On the JVM only: `snappy` using Netty's Snappy *framing* format (`SnappyFrameDecoder`). Payloads must be valid framed Snappy (for example as emitted by Netty's `SnappyFrameEncoder`). Raw Snappy block compression without the framing envelope is not accepted; decompression failures can result in the connection being closed without an HTTP response.
...
IMPORTANT: GraalVM native image builds use Netty substitutions that currently wire only gzip, deflate, and Brotli inbound decoders. Other codings such as `snappy` or `zstd` are not decompressed in native executables even when `quarkus.http.enable-decompression=true`.
