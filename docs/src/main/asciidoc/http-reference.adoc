...
For HTTP/1.x, Quarkus replaces that handler with a subclass that logs a **WARN** (category `io.quarkus.vertx.http.runtime.netty.LoggingHttpContentDecompressor`) and responds with **`400 Bad Request`** if decompression fails, then closes the connection.
HTTP/2 inbound decompression uses Vert.x's HTTP/2 stack and does not go through this handler.
...
* `snappy` using Netty's Snappy *framing* format (`SnappyFrameDecoder`). Payloads must be valid framed Snappy (for example as emitted by Netty's `SnappyFrameEncoder`). Raw Snappy block compression without the framing envelope is not accepted; decompression failures on HTTP/1.x produce `400 Bad Request` with a short plain-text body (and a WARN log); on HTTP/2 the connection may still be closed without a normal HTTP response. Supported on the JVM and in GraalVM native images when inbound decompression is enabled.
* On the JVM only: `zstd` when Netty's Zstd support is available at runtime
...
IMPORTANT: GraalVM native image builds use Netty substitutions for the inbound decompressor. Inbound `zstd` is not wired there and is not decompressed in native executables even when `quarkus.http.enable-decompression=true`. Inbound `gzip`, `deflate`, `br` (when Brotli loads), and framed `snappy` are supported.
